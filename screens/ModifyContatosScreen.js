...
export default function ModifyContatosScreen({ route }) {
  ...
  const removeApi = async () => {
    await axiosInstance.delete("/listaContatos/" + id);
  ...
  const patchApi = async () => {
    await axiosInstance.patch("/listaContatos/" + id, {
      nome: nomeInput,
    ...
    <View className="h-full">
      ...
      <View className="flex p-2">
        ...
        <TextInput
          ...
          className="border-2 rounded-lg p-1 text-black"
        ></TextInput>
      </View>
      <View className="flex p-2">
      ...
      <TouchableOpacity
        className="bg-blue-600 rounded-lg p-4 mx-2 mt-4"
        onPress={patchApi}
      >
        <Text className="text-center">Salvar</Text>
      </TouchableOpacity>
      <TouchableOpacity
        className="bg-red-600 rounded-lg p-4 mx-2 mt-4"
        onPress={removeApi}
      >
